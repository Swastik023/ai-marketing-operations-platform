...
**Plugin version: 3.17.0 | Methodology version: 3.2** | A user-facing guide to running marketing engagements with OmniGrowth Engine

> The 12-Part methodology itself was introduced in v3.0 and remains structurally unchanged through v3.15. v3.2 adds two adjacent commands that pair well with the engagement workflow: **`/omni-growth-engine:check`** (pre-publish quality gate for any deliverable produced inside an engagement) and **`/omni-growth-engine:status`** (engagement progress + brand snapshot). See [docs/v3.2-opt-ins.md](v3.2-opt-ins.md) for the full v3.2 additions.
...
If you have used the plugin only for one-off tasks (`/omni-growth-engine:campaign-plan`, `/omni-growth-engine:content-engine`, etc.), this guide shows you the higher-leverage way to use it: as a methodology that orchestrates the full engagement.
...
```
/omni-growth-engine:content-engine           # Generate content
/omni-growth-engine:email-sequence           # Build an email flow
/omni-growth-engine:competitor-analysis      # Run a competitor audit
/omni-growth-engine:campaign-plan            # Plan a single campaign
```
...
```
/omni-growth-engine:engagement next                           # Confirm current part complete; advance to next
/omni-growth-engine:engagement status                         # See where you are
/omni-growth-engine:engagement four-core <brand> <id>         # Produce Part 3 (61 steps across 4 docs)
/omni-growth-engine:engagement validate <brand> <id>          # Produce Part 5 client validation
/omni-growth-engine:engagement re-run-decision <brand> <id>   # Apply Decision Matrix for Part 6
/omni-growth-engine:engagement growth-plan <brand> <id>       # Produce Part 8 flagship deliverable
/omni-growth-engine:engagement yearly-planner <brand> <id>    # Produce Part 8 operational companion
/omni-growth-engine:engagement loop <brand> <id>              # Part 12 quarterly or ad-hoc brief
```
...
```
/omni-growth-engine:engagement four-core <brand> <id>
```

Or produce one specific document:

```
/omni-growth-engine:engagement four-core <brand> <id> --doc 3.1
```
...
```
/omni-growth-engine:engagement growth-plan <brand> <id>
```
...
```
/omni-growth-engine:engagement yearly-planner <brand> <id>
```
...
```
/omni-growth-engine:engagement loop <brand> <id>
```
...
```
/omni-growth-engine:engagement update-back <brand> <id> --doc 3.1 --reason "Segment X CAC corrected from INR 3,000 to INR 4,800 based on Q2 channel data"
```
...
- [`/omni-growth-engine:engagement`](../commands/engagement.md) — the entry point with all subcommands
...
*OmniGrowth Engine v3.0 — The 12-Part Engagement Methodology. Built by [Indranil Banerjee](https://github.com/swastik-agnihotri). Issues and contributions welcome.*

...
User runs `/omni-growth-engine:content-engine` or asks to draft, write, create, or optimize marketing content.
...
- Create variations for A/B testing? (`/omni-growth-engine:prompt-test`)
- Evaluate quality with the full scoring framework? (`/omni-growth-engine:eval-content`)
- Adapt this content for other channels? (`/omni-growth-engine:content-repurpose`)
- Create an email sequence from this content? (`/omni-growth-engine:email-sequence`)
- Generate social media posts to promote this? (`/omni-growth-engine:social-strategy`)"

## Execution discipline — parallel dispatch

When `/omni-growth-engine:content-engine` is invoked to produce **multiple content formats from a single brief** (e.g. "draft a launch blog post + 3 social posts + email teaser + ad copy"), dispatch the per-format generations in **one message with parallel `Task` calls**. Each format reads the same brief, applies the same brand voice, but produces an independent draft — there is no cross-dependency between, say, the LinkedIn post and the email teaser.
...
3. `/omni-growth-engine:check` quality gate on each draft (parallel by file)

...
[![Stars](https://img.shields.io/github/stars/Swastik023/ai-marketing-operations-platform?style=flat&logo=github&color=yellow)](https://github.com/Swastik023/ai-marketing-operations-platform/stargazers)
[![Forks](https://img.shields.io/github/forks/Swastik023/ai-marketing-operations-platform?style=flat&logo=github&color=blue)](https://github.com/Swastik023/ai-marketing-operations-platform/network/members)
[![Issues](https://img.shields.io/github/issues/Swastik023/ai-marketing-operations-platform?logo=github)](https://github.com/Swastik023/ai-marketing-operations-platform/issues)
[![Last commit](https://img.shields.io/github/last-commit/Swastik023/ai-marketing-operations-platform?logo=github)](https://github.com/Swastik023/ai-marketing-operations-platform/commits/main)
...
> If this saves you time, [give it a star ⭐](https://github.com/Swastik023/ai-marketing-operations-platform/stargazers) — it's the single thing that helps other marketers find it.
...
| **Cursor 2.5+** | In any Cursor Agent chat: `/add-plugin omni-growth-engine@https://github.com/Swastik023/ai-marketing-operations-platform` | `.cursor-plugin/plugin.json` (published Cursor JSON Schema) | Full skills + agents + commands support |
...
| **Google Antigravity 2.0** CLI + IDE | `agy plugin install https://github.com/Swastik023/ai-marketing-operations-platform` | `gemini-extension.json` (at repo root, per Google's reference pattern) | Full skills + hooks support; subagents need `/agent` CLI spawning; slash commands fold into skills via `agy plugin import gemini` |
| **Hermes Agent** (Nous Research) — Desktop + CLI on macOS / Windows / Linux | `hermes plugins install Swastik023/ai-marketing-operations-platform` | `plugin.yaml` + `__init__.py` at repo root (Hermes native spec) | Native plugin — adapter walks `skills/` at register time and exposes all 163 skills via `ctx.register_skill()`. Targets Hermes Desktop v0.15.2+ (public preview June 2 2026). |
| **OpenClaw** (formerly Clawdbot / Moltbot) | `openclaw plugins install git:github.com/Swastik023/ai-marketing-operations-platform` | `openclaw.plugin.json` at repo root (also auto-detects `.claude-plugin/plugin.json` as Claude-compatible bundle) | Native plugin via `openclaw.plugin.json`; `skills` field points at `./skills`. Also installable via ClawHub marketplace (submission pending). |
| **Grok** (xAI Build CLI) | `grok plugin install Swastik023/ai-marketing-operations-platform` — or `grok plugin marketplace add swastik-agnihotri/neels-plugins` then `grok plugin install omni-growth-engine` (append `--trust` to skip the install confirmation) | `.grok-plugin/plugin.json` + `.grok-plugin/marketplace.json` ([Grok Build](https://docs.x.ai/build/features/skills-plugins-marketplaces) also reads the Claude Code manifests for compatibility; the native pair is the first-class lane) | Full skills support |
...
|---|---|---|
| [Goose](https://block.github.io/goose) | Block (Square) | `goose skills install github.com/Swastik023/ai-marketing-operations-platform/skills` |
| [OpenHands](https://openhands.dev) | Open Hands (cloud agents) | Mount this repo's `skills/` via the OpenHands skills config |
| [OpenCode](https://opencode.ai) | sst | `opencode skills import github:Swastik023/ai-marketing-operations-platform` |
| [Junie](https://junie.jetbrains.com) | JetBrains | Drop `skills/` into your project; Junie auto-discovers |
| [Gemini CLI](https://geminicli.com) | Google | `gemini skills add github:Swastik023/ai-marketing-operations-platform` |
...
| [Amp](https://ampcode.com) | Sourcegraph | `amp skills add github:Swastik023/ai-marketing-operations-platform` |
...
git clone --depth=1 https://github.com/Swastik023/ai-marketing-operations-platform.git
...
If you run into a platform-specific install snag, file a [GitHub issue](https://github.com/Swastik023/ai-marketing-operations-platform/issues) — we'll add platform-specific docs as users report patterns.
...
[File an issue](https://github.com/Swastik023/ai-marketing-operations-platform/issues) with the citation. Privacy and AI law change quarterly — DM Pro is actively maintained against the May 2026 reality but enforcement actions and amendments keep coming.
...
Use the full Git URL form: `/add-plugin omni-growth-engine@https://github.com/Swastik023/ai-marketing-operations-platform`. Cursor's marketplace integration is fastest, but the Git URL form always works.
...
Use the `git:` install scheme: `openclaw plugins install git:github.com/Swastik023/ai-marketing-operations-platform`. If you used another scheme and it failed, the fallback is: `cd ~/.openclaw/plugins && git clone https://github.com/Swastik023/ai-marketing-operations-platform && openclaw plugins enable omni-growth-engine`.
...
**Still stuck?** [Open an issue](https://github.com/Swastik023/ai-marketing-operations-platform/issues) with the exact error message + platform name + version. We respond within a few days.
...
| [ContentForge](https://github.com/Swastik023/contentforge) | Publication-ready content via 10-phase pipeline, fact-checker, 35-pattern AI-detection humanizer, .docx export with C2PA signing |
| [SocialForge](https://github.com/Swastik023/socialforge) | Social media calendar with AI image (Vertex AI Nano Banana Pro) + video (WaveSpeed Kling v3.0 Pro) generation, C2PA signing |
...
<a href="https://www.star-history.com/?type=date&repos=swastik-agnihotri%2Fomni-growth-engine">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=Swastik023/ai-marketing-operations-platform&type=date&theme=dark&legend=top-left&sealed_token=AqnW48iQwpZNQCx5Ncz_reoaRoDWKEEG-sZXQohHllcyAFnSDLdSJVqEoTci2Y8ognOBGCUrCY9eU3yUIW_YG7TwVhwub90B7qGh-9qlJgGjfFQQbp__puZDwereB6S-SQzbcK8B68Z-izIHjTt1DFPa5YxfuWlFF8MhrLdIEhFdU2x-cwHzmGWYYhBl" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=Swastik023/ai-marketing-operations-platform&type=date&legend=top-left&sealed_token=AqnW48iQwpZNQCx5Ncz_reoaRoDWKEEG-sZXQohHllcyAFnSDLdSJVqEoTci2Y8ognOBGCUrCY9eU3yUIW_YG7TwVhwub90B7qGh-9qlJgGjfFQQbp__puZDwereB6S-SQzbcK8B68Z-izIHjTt1DFPa5YxfuWlFF8MhrLdIEhFdU2x-cwHzmGWYYhBl" />
    <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=Swastik023/ai-marketing-operations-platform&type=date&legend=top-left&sealed_token=AqnW48iQwpZNQCx5Ncz_reoaRoDWKEEG-sZXQohHllcyAFnSDLdSJVqEoTci2Y8ognOBGCUrCY9eU3yUIW_YG7TwVhwub90B7qGh-9qlJgGjfFQQbp__puZDwereB6S-SQzbcK8B68Z-izIHjTt1DFPa5YxfuWlFF8MhrLdIEhFdU2x-cwHzmGWYYhBl" />
  </picture>
</a>

If DM Pro saves your team time, [⭐ star the repo](https://github.com/Swastik023/ai-marketing-operations-platform/stargazers) — it's the single most useful thing you can do to help other marketing teams discover it.
...
- 📦 **Other plugins:** [ContentForge](https://github.com/Swastik023/contentforge) · [SocialForge](https://github.com/Swastik023/socialforge)
- 💬 **Discussions:** [GitHub Discussions](https://github.com/Swastik023/ai-marketing-operations-platform/discussions)
- 🐛 **Bug reports:** [GitHub Issues](https://github.com/Swastik023/ai-marketing-operations-platform/issues)
...
If DM Pro saves your team time, [⭐ star the repo](https://github.com/Swastik023/ai-marketing-operations-platform/stargazers) — it's the single most useful thing you can do to help other marketing teams discover it. Sharing it on **LinkedIn** or **X** helps people discover the work too.

---

## Contributing

PRs welcome — especially on compliance rules (privacy and AI law change fast), industry profiles, and channel-specific updates. See [CONTRIBUTING.md](CONTRIBUTING.md) for the contribution workflow, [`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md) for the PR checklist, and [TESTING-GUIDE.md](TESTING-GUIDE.md) for per-phase test checklists. All contributors are expected to follow the [Code of Conduct](CODE_OF_CONDUCT.md). Security issues: please use [Private Security Advisories](https://github.com/Swastik023/ai-marketing-operations-platform/security/advisories/new) per [SECURITY.md](SECURITY.md) — do not file public issues for vulnerabilities.
...
<sub>Made with care by [Indranil Banerjee](https://indranil.in) · Powered by Anthropic Claude · MIT-licensed · [⭐ Star the repo](https://github.com/Swastik023/ai-marketing-operations-platform) if it helps you</sub>

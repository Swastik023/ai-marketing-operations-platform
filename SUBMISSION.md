...
**Repository:** https://github.com/swastik-agnihotri/omni-growth-engine
**Marketplace:** https://github.com/swastik-agnihotri/neels-plugins
...
OmniGrowth Engine is an end-to-end engagement methodology for marketing teams running on Claude Code & Cowork. Every brand engagement runs through a canonical 12-Part Strategy Flow producing the Four Core Documents (61 explicit steps), the Two-Views Model (v1 unbiased + v2 client-validated), the Decision Matrix for selective re-runs, the Update-Back Rule for in-life corrections, and a Living Project Instruction File that keeps all downstream skills synchronized to the latest source-of-truth.
...
/omni-growth-engine:check drafts/q3-launch-blog.md --brand acme-corp --schema blog_post
```
...
```
/omni-growth-engine:competitor-analysis HubSpot Marketo Klaviyo
```
...
```
/omni-growth-engine:switch-brand pharma-client-a
/omni-growth-engine:engagement
# ... later
/omni-growth-engine:switch-brand bfsi-client-b
/omni-growth-engine:engagement
```
...
**Testing account:** Reviewers can install the plugin from the public marketplace at `swastik-agnihotri/neels-plugins` and create a sample brand interactively via `/omni-growth-engine:brand-setup` (no bundled `config/` template file ships — profiles are generated by the setup skill). No paid services or API keys required for the default knowledge-only mode (covers all 163 skills + 24 agents + 169 reference files). Optional Python scoring scripts use `nltk` and `textstat` (free, open-source).

**Sample brand profile:** Run `/omni-growth-engine:brand-setup` (Quick or Full mode) to generate a fully populated `profile.json` under `~/.claude-marketing/brands/<slug>/` showing every supported field (voice, audience, channels, guardrails, jurisdictions, etc.). There is no committed `examples/` or `config/` sample directory in the repo.

## 7. Ownership verification

- **Repo:** github.com/swastik-agnihotri/omni-growth-engine — owned by GitHub user @swastik-agnihotri (the submitter)
- **Marketplace:** github.com/swastik-agnihotri/neels-plugins — same owner
...
- **Trademarks:** "OmniGrowth Engine" and "Neelverse" are submitter's marks. No third-party trademarks used in the plugin name, command names, or UI strings.
...
| Design guidelines | ✓ Plugin name uses canonical Claude Code namespace `/omni-growth-engine:<command>`; no bare slash commands that conflict with other plugins; README is onboarding-first per v3.3 restructure; all command frontmatter has `description`, `argument-hint`, `allowed-tools` per Claude Code spec. |
...
- Plugin ships zero global hooks (`hooks/hooks.json` is empty) and zero auto-connecting MCP servers (`.mcp.json` is empty until user opt-in via `/omni-growth-engine:connect`). Multi-plugin coexistence by design.
...
1. `/omni-growth-engine:status` output for a populated brand (shows the brand snapshot UX)
2. A completed `/omni-growth-engine:engagement` Part 5 Client Validation Document (shows the methodology output)
3. A `/omni-growth-engine:check` BLOCKED decision with CRITICAL issues highlighted (shows the quality gate UX)
4. `/omni-growth-engine:competitor-analysis` final report (shows multi-dimensional output)
...
2. Plugin name: `omni-growth-engine`
3. Marketplace source: `github.com/swastik-agnihotri/neels-plugins` (custom marketplace) OR `github.com/swastik-agnihotri/omni-growth-engine` (direct repo)

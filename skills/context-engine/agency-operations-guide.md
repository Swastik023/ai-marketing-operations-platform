...
This reference defines standard operating procedures for agencies managing multiple client brands through the OmniGrowth Engine plugin. It covers onboarding, portfolio health tracking, SOP frameworks, reporting cadences, cross-client intelligence, credential isolation, and white-labeling rules.
...
|---|---|---|---|---|
| 1 | **Create brand profile** | `/omni-growth-engine:brand-setup` | 10-15 min | Walk through identity, voice, audiences, competitors, goals. Produces `profile.json` at `~/.claude-marketing/brands/{slug}/`. |
| 2 | **Create credential profile** | `/omni-growth-engine:credential-switch` | 5-10 min | Set up platform API keys and tokens. Stored at `~/.claude-marketing/credentials/{slug}.json`. Never share credentials across brands. |
| 3 | **Connect CRM** | `/omni-growth-engine:crm-sync` | 5-10 min | Link CRM (HubSpot, Salesforce, Pipedrive). Validate connection with a test query. Import initial contact counts. |
...
| 5 | **Assign SOPs from library** | `/omni-growth-engine:sop-library` | 5 min | Select applicable SOP templates per client's service package (content, paid media, reporting, CRM, SEO). Customize cadences. |
| 6 | **Assign team members** | `/omni-growth-engine:team-assign` | 5 min | Map roles: account manager, strategist, content creator, media buyer, analyst. Set notification preferences. |
...
| **Profile switching** | `/omni-growth-engine:credential-switch` or `setup.py --switch-credentials` | Switches the active credential profile. All subsequent MCP and API calls use the new profile's keys. |
...
- After every credential change, run `/omni-growth-engine:validate-profile --brand {brand}` — it probes every connector referenced by the profile and reports pass / fail / error-class without ever printing credential values. Add `--connectors slack,hubspot` to probe only the rotated subset.
...
| Element | Rule |
|---|---|
| **Logo** | Use the agency's logo on all client-facing reports. Never use the OmniGrowth Engine plugin branding. |

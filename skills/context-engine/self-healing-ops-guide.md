...
Logs stored at `~/.claude-marketing/brands/{slug}/ops-log.json` and surfaced via `/omni-growth-engine:campaign-status` and `/omni-growth-engine:anomaly-scan` commands.
...
- **Tier 1:** Auto-corrected immediately. Summary in next `/omni-growth-engine:campaign-status` report.

...
6. **Update local index**: Run `memory-manager.py --action log-stored` to register the new entry in the local content hash registry with storage ID, vector DB reference, timestamp, and priority. Update sync state so future `/omni-growth-engine:sync-memory` runs skip this item as already persisted.
...
- **Retrieval hint**: Two to three example search queries that would surface this entry — so the user knows exactly how to find it later via `/omni-growth-engine:search-knowledge`
